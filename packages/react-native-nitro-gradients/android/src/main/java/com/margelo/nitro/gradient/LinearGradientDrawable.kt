...
class LinearGradientDrawable : Drawable() {
    ...
    fun setStart(start: VectorR, width: Int, height: Int, density: Float) {
        val x = toCoordinate(start.x, width, density)
        val y = toCoordinate(start.y, height, density)
    ...
    fun setEnd(end: VectorR, width: Int, height: Int, density: Float) {
        val x = toCoordinate(end.x, width, density)
        val y = toCoordinate(end.y, height, density)
        if (endX != x || endY != y) {
            endX = x
            endY = y
            isDirty = true
    ...
    private fun toCoordinate(variant: Variant_String_Double, dimension: Int, density: Float): Float {
        return when (variant) {
            ...
            is Variant_String_Double.Second -> {
                // Numeric values are absolute points (like React Native), convert to pixels then normalize
                if (dimension > 0) {
                    (variant.value.toFloat() * density) / dimension.toFloat()
                } else {
                    0f
                }
            }
