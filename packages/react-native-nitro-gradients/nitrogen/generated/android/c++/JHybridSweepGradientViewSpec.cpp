...
namespace margelo::nitro::gradient {
  ...
  // Methods
  void JHybridSweepGradientViewSpec::update(const std::vector<double>& colors, const std::optional<std::vector<double>>& positions, const std::optional<Vector>& center, std::optional<double> blur, const std::optional<std::string>& tileMode) {
    static const auto method = _javaPart->javaClassStatic()->getMethod<void(jni::alias_ref<jni::JArrayDouble> /* colors */, jni::alias_ref<jni::JArrayDouble> /* positions */, jni::alias_ref<JVector> /* center */, jni::alias_ref<jni::JDouble> /* blur */, jni::alias_ref<jni::JString> /* tileMode */)>("update");
    method(_javaPart, [&]() {
      size_t __size = colors.size();
      jni::local_ref<jni::JArrayDouble> __array = jni::JArrayDouble::newArray(__size);
      __array->setRegion(0, __size, colors.data());
